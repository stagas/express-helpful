...
var render = exports.render = function(view, context, locals) {
  var req = context.locals.req
    ...
    , viewHash = hash(req.method + req.url + req.headers.ip + req.sessionID + view)
...
var cache = exports.cache = function() {
  return function(req, res, next) {
    ...
    res.render = function(view, locals) {
      var viewHash = hash(req.method + req.url + req.headers.ip + req.sessionID + view)
        , urlHash = hash(req.url + req.headers.ip + req.sessionID)

      res.headers["Date"] = new Date().toUTCString()
      res.headers["Last-Modified"] = _cache[viewHash] || res.headers["Date"]
      ...
      switch (typeof req.cacheExpire) {
        ...
        case 'number':
          if (!cacheTimeout[viewHash]) { 
            cacheTimeout[viewHash] = setTimeout(function() {
              delete _cache[viewHash]
              delete cacheView[viewHash]
              delete cacheTimeout[viewHash]
            }, req.cacheExpire)
          }
          break
        default:
      ...
      var out = render(locals.layout || 'layout', req.context, locals)
      
      res.writeHead(200, res.headers)
      res.end(out)
      
      if (req.cacheExpire) {
        _cache[viewHash] = res.headers["Last-Modified"]
        cacheView[viewHash] = out
        if (typeof cacheList[urlHash] !== 'object') cacheList[urlHash] = { u: hash(req.headers.ip + req.sessionID) }
        cacheList[urlHash][viewHash] = true
      }
    }
